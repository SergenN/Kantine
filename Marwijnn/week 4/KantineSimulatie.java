...
public class KantineSimulatie {
  ...
  public void simuleer(int dagen) {
    ...
    for(int i=0;i<dagen;i++) {
      ...
      int aantalpersonen= getRandomValue(MIN_PERSONEN_PER_DAG, MAX_PERSONEN_PER_DAG);
        ...
        for(int j=0;j<aantalpersonen;j++) {
          ...
          // bedenk hoeveel artikelen worden gepakt
            ...
            if      (rand == 1) persoon = new KantineMedewerker();
            else if (rand <= 11)persoon = new Docent();
            else                persoon = new Student();
      ...
      System.out.println("    totaal geld in kassa : " + 
             kantine.getKassa().hoeveelheidGeldInKassa());
      System.out.println("    totaal artikelen verkocht : " + 
    ...
    System.out.println("Gemiddeld aantal verkocht : " + Administratie.berekenGemiddeldAantal(aantal));
    System.out.println("\nDag Omzet:");
    double[] dagomzet = Administratie.berekenDagOmzet(omzet);
    for (int i = 0; i < dagomzet.length; i++){
        System.out.println("Dag " + (i + 1) + ": " + dagomzet[i]);
    }
  }
